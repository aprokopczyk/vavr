...
class OptionTest {
    ...
    // ---- error that may be thrown in the empty case
    ...
    void shouldVerifyBasicSomeProperties() {
        final var some = Option.some(SOME_VALUE);
    ...
    void shouldFlatMapIfNone() {
        assertSame(NONE, NONE.flatMap(ignored -> SOME));
    }
    ...
    void shouldGetOrElseGetOnSome() {
        assertSame(SOME_VALUE, SOME.getOrElseGet(() -> null));
    }
    ...
    @Test
    void shouldMapNoneToNull() {
        assertSame(NONE, NONE.map(s -> null));
    }


    @Test
    void shouldMapNoneToNonNull() {
        assertSame(NONE, NONE.map(s -> s + "!"));
    }
    ...
    @Test
    void shouldConvertSomeOfNonNullToTry() {
        assertEquals(Try.success(SOME_VALUE), SOME.toTry(() -> ERROR));
    }

    @Test
    void shouldConvertSomeOfNullToTry() {
